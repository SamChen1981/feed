...
//
// If d <= 0, connRequest never timeout.
...
func (db *DB) conn(strategy connReuseStrategy) (*driverConn, error) {
	...
	if db.maxOpen > 0 && db.numOpen >= db.maxOpen {
		...
		if db.connWaitTimeout > 0 {
			select {
			case ret, ok := <-req:
				return db.processConnResponse(ret, ok, lifetime)
			case <-time.After(db.connWaitTimeout):
				db.mu.Lock()
				if c := len(db.connRequests); c > 0 {
					index := -1
					for i, connReq := range db.connRequests {
						if connReq == req {
							index = i
							break
						}
					}
					if index >= 0 { // req found
						if index < c-1 {
							copy(db.connRequests[index:], db.connRequests[index+1:])
						}
						db.connRequests[c-1] = nil
...
func (db *DB) processConnResponse(ret connRequest, ok bool, lifetime time.Duration) (*driverConn, error) {
	if !ok {
		return nil, errDBClosed
	}
